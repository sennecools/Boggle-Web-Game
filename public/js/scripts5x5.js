...
async function addWordButton() {
    ...
    if (data.title == "No Definitions Found") {
        ...
        if (existingWord) {
            resultaat.innerText = `${word}: word has already been used`;
            undoButtonClick();
            return;
        }
...
letterButtons.forEach(button => {
    button.addEventListener('click', () => {
        ...
        const isTopRow = square <= 5;
        const isBottomRow = square >= 21;
        const isLeftColumn = square % 5 === 1;
        const isRightColumn = square % 5 === 0;
        ...
        if (!isTopRow && !isLeftColumn) {
            const topLeftButton = document.querySelector(`[data-square="${square - 6}"]`);
            topLeftButton.disabled = false;
        }

        if (!isTopRow) {
            const topButton = document.querySelector(`[data-square="${square - 5}"]`);
            topButton.disabled = false;
        }

        if (!isTopRow && !isRightColumn) {
            const topRightButton = document.querySelector(`[data-square="${square - 4}"]`);
        ...
        if (!isBottomRow && !isLeftColumn) {
            const bottomLeftButton = document.querySelector(`[data-square="${square + 4}"]`);
            bottomLeftButton.disabled = false;
        }

        if (!isBottomRow) {
            const bottomButton = document.querySelector(`[data-square="${square + 5}"]`);
            bottomButton.disabled = false;
        }

        if (!isBottomRow && !isRightColumn) {
            const bottomRightButton = document.querySelector(`[data-square="${square + 6}"]`);
